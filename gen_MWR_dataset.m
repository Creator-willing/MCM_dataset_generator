...
% Reproduce the paper: Blind multicarrier waveform recognition based on spatial-temporal learning neural networks
%% Input:
%% Output: MWR_all_AMC.mat
%% Author: Mingkun Li
%% Date: 2025/1/4
...
Simulation_MonteCarloRepetitions  = 2000;                                % Number of Monte Carlo repetitions over which we take the average                  
...
%% 生成数据
mod_list = ["xOTFS","xAFDM","xSC","xGFDM","xFOFDM","xUFMC","xFBMC","xOFDM","xWOLA"];
...
for i_rep = 1:Simulation_MonteCarloRepetitions
    ...
    elseif mod_name == "xSC"
        BinaryDataStream_SC     = randi([0 1], SC_M_bits * SC_N_syms_perfram,1);
        x_SC = BinaryDataStream_SC;
        s_moded = qammod(reshape(x_SC,SC_M_bits,[]), QAM_ModulationOrder,'gray','InputType','bit', 'UnitAveragePower', true);
        s_moded = s_moded';
    elseif mod_name == "xOTFS"
        BinaryDataStream_OTFS     = randi([0 1], OTFS_M_bits * OTFS_NumberOfSubcarriers * OTFS_NumberOfSymbolsInTime,1);
    ...
    elseif mod_name == "xAFDM"
        BinaryDataStream_AFDM     = randi([0 1], AFDM_M_bits * AFDM_NumberOfSubcarriers * AFDM_NT,1);
    ...
    elseif mod_name == "xGFDM"
        [GFDM_Tx_real, GFDM_Tx_imag] = GFDM_Coder(GFDM_NumberOfSubcarriers,GFDM_NumberOfSymbolsInTime,GFDM_pulse_shape,GFDM_rolloff,GFDM_overlapping,GFDM_cyclic_prefix_length,4);
    ...
    % 将数据保持在本地mat文件
    if mod_index == 1
        save('MWR_all_AMC.mat',mod_list(mod_index),"-append")
    else
        save('MWR_all_AMC.mat',mod_list(mod_index),"-append"); 
    end
end
